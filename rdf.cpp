...
double configuration::get_dist(int i, int j)
{
    ...
    xyz rij_vec; // Distance vector between atoms i and j

    // Get the distance vector between atoms i and j
    rij_vec.x = coords[j].x - coords[i].x;
    rij_vec.y = coords[j].y - coords[i].y;
    rij_vec.z = coords[j].z - coords[i].z;

    // Apply the minimum image convention
    rij_vec.x -= std::round(rij_vec.x / boxdims.x) * boxdims.x;
    rij_vec.y -= std::round(rij_vec.y / boxdims.y) * boxdims.y;
    rij_vec.z -= std::round(rij_vec.z / boxdims.z) * boxdims.z;
...
int get_rdf_bin(int nbins, double binw, double dist)
{
    ...
    int bin_index = static_cast<int>(dist / binw);
    if (bin_index >= nbins) {
...
int main(int argc, char* argv[])
{
    ...
    double  binw;   //0.2; // Binwidth for RDF computation - determined based on boxlength
    ...
    while(frame.read_frame()) // Read and populate the number integral
    {
        ...
        {
            binw = frame.boxdims.x / 300.0; // Set the bin width 
            nbins = static_cast<int>((frame.boxdims.x / 2.0) / binw); // Set the number of bins based on the system box length and the requested bin width. 
    ...
    rdfstream << "# Distance (Ang)      Number Integral (atoms)     RDF (unitless)" << endl;
     ...
     for (int i=0; i<nbins-1; i++)
     {
         ...
         double shell_vol           = (4.0 / 3.0) * pi * (pow((i + 1) * binw, 3) - pow(i * binw, 3));
         double shell_density       =  num_int[i] / shell_vol / (nframes - 1)/ frame.natoms;
         double shell_density_ideal = frame.density;
